...
const Pro: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const proLeads = allLeads.filter(
          (lead) => customerTypes[lead.id!] === "Pro"
        );
        setLeads(proLeads);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Pro Customers</h1>
        <p className="text-muted-foreground">
          {leads.length} pro customer{leads.length !== 1 ? "s" : ""}
        </p>
      </div>
